...
if [ "$NAME" == "all" ]
then
  rclone copy --no-traverse "$SAVEFOLDER" ${REMOTENAME}:${SERVERNAME}/stream-recorder/${SAVEFOLDER} --include-from rcloneinclude.txt --buffer-size 32M --transfers 6 --low-level-retries 200 -v > "./log/rclone_${LOG_PREFIX}.log" 2>&1
elif [ -z "$SITE" ]
then
  rclone copy --no-traverse "${SAVEFOLDER}/${NAME}" ${REMOTENAME}:${SERVERNAME}/stream-recorder/${SAVEFOLDER}/${NAME} --include-from rcloneinclude.txt --buffer-size 32M --transfers 6 --low-level-retries 200 -v > "./log/rclone_${LOG_PREFIX}.log" 2>&1
else
  rclone copy --no-traverse "${SAVEFOLDER}/${NAME}/${SITE}" ${REMOTENAME}:${SERVERNAME}/stream-recorder/${SAVEFOLDER}/${NAME}/${SITE} --include-from rcloneinclude.txt --buffer-size 32M --transfers 6 --low-level-retries 200 -v > "./log/rclone_${LOG_PREFIX}.log" 2>&1  
fi
